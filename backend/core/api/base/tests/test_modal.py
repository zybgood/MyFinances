...
def test_open_modal_leave_team_context(mock_render):
    ...
    user = User.objects.create_user(username="leaveuser", email="leaveuser@example.com", password="leavepass")
    org = Organization.objects.create(name="Test Org", leader=user)
    org.members.add(user)
    request = factory.get("/fake-url/")
    request.user = user
    response = open_modal(request, modal_name="some_modal", context_type="leave_team", context_value=org.id)  # 再也不会真的去找模板
    assert response.status_code == 200
    mock_render.assert_called_once()
...
def test_open_modal_receipt_edit_context(mock_render):
    ...
    user = User.objects.create_user(username="receiptuser", email="receipt@example.com", password="receiptpass")
    receipt = Receipt.objects.create(
        owner=user, name="Test Receipt", date=None, merchant_store="Test Store", purchase_category="Food", total_price=12.5
    )
    request = factory.get("/fake-url/")
    request.user = user
    response = open_modal(request, modal_name="edit_receipt_modal", context_type="edit_receipt", context_value=receipt.id)
    assert response.status_code == 200
...
def test_open_modal_profile_picture(mock_render):
    ...
    user = User.objects.create_user(username="u1", email="u1@example.com", password="123")
    request = factory.get("/fake-url/")
    request.user = user
...
def test_open_modal_edit_invoice_to(mock_render):
    ...
    user = User.objects.create_user(username="u4", email="u4@example.com", password="123")
    client = Client.objects.create(owner=user, name="A", email="c@example.com", company="ACorp", address="addr")
    invoice = Invoice.objects.create(owner=user, client_to=client, date_due=timezone.now() + timedelta(days=30))
    request = factory.get("/fake-url/")
    request.user = user
...
def test_open_modal_invoice_reminder_with_access(mock_render):
    ...
    user = User.objects.create_user(username="u5", email="u5@example.com", password="123")
    client = Client.objects.create(owner=user, email="c2@example.com")
    invoice = Invoice.objects.create(owner=user, client_to=client, date_due=timezone.now() + timedelta(days=30))
